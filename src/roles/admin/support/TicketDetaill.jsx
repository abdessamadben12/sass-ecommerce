...
const TicketDetail = () => {
 ...
 useEffect(() => {
  ...
  const getStatusColor = (status) => {
    const colors = {
      'new': 'bg-[#E6F6FF] text-[#008ECC] border-[#B9E7FF]',
  ...
  return (  loading ? (<Loading/>) :    <div className="min-h-screen bg-slate-100">
      {/* Header */}
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button onClick={() => navigate(-1)} className="p-2 text-slate-400 hover:text-[#008ECC] hover:bg-[#E6F6FF]">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white shadow-sm border border-slate-200 p-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <span className={`inline-flex items-center gap-1 px-3 py-1 text-sm font-medium border ${getStatusColor(ticket.status)}`}>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-[#008ECC] text-white hover:bg-[#007bb3] transition-colors"
                  >
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {ticket?.attachments.map((attachment) => (
                      <div
                        key={attachment.id}
                        className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 hover:bg-[#E6F6FF] transition-colors"
                      >
                        <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          className="p-1 text-slate-400 hover:text-[#008ECC]">
            ...
            <div className="bg-white shadow-sm border border-slate-200">
              ...
              <div className="p-6 space-y-6 ">
                ...
                {ticket.message_replay?.map((reply) => (
                  <div
                    key={reply?.id}
                    className={`border-l-4 pl-4 bg-white border-[#B9E7FF] p-4 text-wrap shadow`}>
                    <div className="flex items-center  ">
                <button
                  onClick={() => deleteReply(reply?.id)}
                  className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors duration-200"
                  title="Supprimer"
                >
                  <Trash className="w-4 h-4" />
                </button>
                <span
                  className={`p-2 cursor-text transition-colors duration-200 ${
                    reply?.is_read 
                      ? 'text-slate-400' 
                      : 'text-[#008ECC] '
                  }`}
                >
                  {reply?.is_read ? <MailOpen className="w-4 h-4" /> : <Mail className="w-4 h-4" />}
                   ...
                   <div className=" flex flex-col items-start justify-between mb-4 bg-slate-100 p-4">
                    {/* message */}
      ...
      <span
        ...
        className="cursor-pointer flex justify-strat mt-2 text-[#008ECC] text-sm "
      >
  ...
  { reply?.attachments?.length > 0 && reply?.attachments?.map((attachment,key)=>{
    return <div
                          key={key}
                          className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 hover:bg-[#E6F6FF] transition-all duration-200 group"
                        >
                          <div className="flex items-center gap-4">
                            <div className="p-2 bg-white border border-slate-200 group-hover:border-[#B9E7FF] transition-colors">
                              {getFileIcon(attachment.type)}
                            </div>
                            <div>
                          ...
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button className="p-2 text-slate-400 hover:text-[#008ECC] hover:bg-white transition-colors">
                            ...
                            <a
                              href={`${apiBase}/api/download-file?url=${encodeURIComponent(attachment.path)}&name=${encodeURIComponent(attachment.name)}`}
                             className="p-2 text-slate-400 hover:text-green-600 hover:bg-white transition-colors">
                              <Download className="w-4 h-4" />
                            </a>
                          </div>
                        </div>
           ...
           <form>
             <div className="bg-white shadow-sm border border-slate-200 p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select value={statusValue} onChange={(e) => setStatusValue(e.target.value)} className="w-full px-3 py-2 border border-slate-300 focus:ring-2 focus:ring-[#008ECC] focus:border-[#008ECC]">
                ...
                <div>
                  ...
                  <select value={priorityValue} onChange={(e) => setPriorityValue(e.target.value)} className="w-full px-3 py-2 border border-slate-300 focus:ring-2 focus:ring-[#008ECC] focus:border-[#008ECC]">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 p-2 border border-slate-200">
              ...
              <div className='flex justify-end'>
                ...
                className="flex items-center  gap-2 px-3 py-2 bg-[#008ECC] mt-2 text-white hover:bg-[#007bb3] transition-colors"
            ...
            <div className="bg-white shadow-sm border border-slate-200 p-6">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 bg-emerald-100 flex items-center justify-center text-sm font-medium text-emerald-800">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span
  className={`text-gray-900 px-2 py-0 ${
  ...
  {ticket.user?.status}
              ...
              <div className="mt-4 pt-4 border-t border-slate-200">
                ...
                onClick={()=>navigate(`/admin/detaill-user/${ticket.user?.id}`)}
                 className="w-full flex items-center justify-center gap-2 px-3 py-2 text-[#008ECC] hover:bg-[#E6F6FF] transition-colors">
